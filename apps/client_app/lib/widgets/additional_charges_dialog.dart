...
class AdditionalChargesDialog extends StatefulWidget {
  const AdditionalChargesDialog({super.key, required this.session});
...
class _AdditionalChargesDialogState extends State<AdditionalChargesDialog> {
              ...
              children: [
                ...
                TextField(
                  ...
                  decoration: const InputDecoration(
                    ...
                    hintText: 'Example: Home Delivery',
                    border: OutlineInputBorder(),
                  ),
                ),
                const SizedBox(height: 10),
                    ...
                    Expanded(
                      child: TextField(
                        ...
                        decoration: const InputDecoration(
                          ...
                          helperText: 'Editable again on each invoice.',
                          border: OutlineInputBorder(),
                        ),
                      ),
                    ),
              ...
              onPressed: () {
                ...
                if (chargeName.isEmpty ||
                  ...
                  ScaffoldMessenger.of(dialogContext).showSnackBar(
                    const SnackBar(
                      content: Text(
                        'Enter a name, non-negative amount and tax from 0 to 100.',
                      ),
                    ),
                  );
                  return;
  ...
  Future<void> _setActive(Map<String, dynamic> charge, bool active) async {
    final id = charge['id']?.toString() ?? '';
    final variantId = charge['service_variant_id']?.toString() ?? '';

    if (id.isEmpty || variantId.isEmpty) {
      setState(() => _error = 'Charge catalogue row is incomplete.');
      return;
    }
    ...
    try {
      await _commercial.saveChargeCatalog(
        tenantId: widget.session.business.id,
        chargeId: id,
        code: charge['code']?.toString() ?? 'CHARGE',
        name: charge['name']?.toString() ?? 'Additional Charge',
        chargeKind: charge['charge_kind']?.toString() ?? 'other',
        serviceVariantId: variantId,
        defaultQuantity: _number(charge['default_quantity']) <= 0
            ? 1
            : _number(charge['default_quantity']),
        active: active,
      );
      await _load();
    } catch (error) {
                  ...
                  const Expanded(
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(
                          'One shared catalogue for Client, POS and Restaurant billing.',
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          Container(
                            ...
                            child: Text(
                              'Each charge is a GST-classified service item. '
                              'The default amount can be changed on an invoice '
                              ...
                              (charge) => Container(
                                ...
                                child: Row(
                                  children: [
                                    ...
                                    Switch(
                                      value: charge['active'] != false,
                                      onChanged: _saving
                                          ? null
                                          : (value) =>
                                                _setActive(charge, value),
                                    ),
                                  ],
                                ),
                              ),
