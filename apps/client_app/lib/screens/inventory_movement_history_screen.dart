...
class InventoryMovementHistoryScreen extends StatefulWidget {
  ...
  @override
  State<InventoryMovementHistoryScreen> createState() =>
      _InventoryMovementHistoryScreenState();
}

class _InventoryMovementHistoryScreenState
    extends State<InventoryMovementHistoryScreen> {
  ...
  Widget build(BuildContext context) {
    final incoming = _rows
        .where((r) => _n(r['base_quantity_delta']) > 0)
        .fold<double>(0, (a, r) => a + _n(r['base_quantity_delta']));
    final outgoing = _rows
        .where((r) => _n(r['base_quantity_delta']) < 0)
        .fold<double>(0, (a, r) => a + _n(r['base_quantity_delta']).abs());
    return Scaffold(
      appBar: AppBar(
          ...
          children: [
            ...
            Expanded(
              child: _rows.isEmpty && !_loading
                  ? const Center(
                      child: Text('No inventory movements in this period.'),
                    )
                  : ListView.separated(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        return ListTile(
                          ...
                          title: Text(
                            '${row['product_name'] ?? 'Product'} • ${row['movement_type'] ?? ''}',
                          ),
                          subtitle: Text(
                            [
