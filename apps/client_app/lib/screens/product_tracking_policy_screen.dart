...
class ProductTrackingPolicyScreen extends StatefulWidget {
  ...
  const ProductTrackingPolicyScreen({
    super.key,
  ...
  @override
  State<ProductTrackingPolicyScreen> createState() =>
      _ProductTrackingPolicyScreenState();
}

class _ProductTrackingPolicyScreenState
    extends State<ProductTrackingPolicyScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      try {
        reconciliation = await _service.reconciliation(
          tenantId: _tenantId,
          variantId: widget.variantId,
        );
      } catch (_) {}
  ...
  void _generateExistingSerials() {
    ...
    final stock = (reconciliation['stock_quantity'] as num?)?.toDouble() ?? 0;
    final tracked =
        (reconciliation['tracked_quantity'] as num?)?.toDouble() ?? 0;
    final remaining = stock - tracked;
    if (remaining <= 0.000001 || remaining != remaining.truncateToDouble()) {
      setState(() {
                ...
                child: ConstrainedBox(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      const Text(
                        'Choose how physical stock is traced. The normal stock ledger remains the quantity/accounting source of truth.',
                      ),
                            ...
                            children: [
                              ...
                              DropdownButtonFormField<String>(
                                ...
                                items: const [
                                  DropdownMenuItem(
                                    value: 'none',
                                    child: Text('No serial / batch tracking'),
                                  ),
                                  DropdownMenuItem(
                                    value: 'serial',
                                    child: Text('Serial number tracking'),
                                  ),
                                  DropdownMenuItem(
                                    value: 'batch',
                                    child: Text('Batch / lot tracking'),
                                  ),
                                ],
                                onChanged: _saving
                                    ? null
                              ...
                              if (_mode == 'batch') ...[
                                ...
                                SwitchListTile(
                                  ...
                                  value: _requireExpiry,
                                  onChanged: (v) =>
                                      setState(() => _requireExpiry = v),
                                  title: const Text(
                                    'Require expiry date on every received batch',
                                  ),
                                ),
                                SwitchListTile(
                                  contentPadding: EdgeInsets.zero,
                                  value: _allowExpired,
                                  onChanged: (v) =>
                                      setState(() => _allowExpired = v),
                                  title: const Text(
                                    'Allow sale from expired batches',
                                  ),
                                  subtitle: const Text(
                                    'Normally keep this off. FEFO skips expired stock.',
                                  ),
                                ),
                              ],
                            ],
                          ...
                          child: Padding(
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                const SizedBox(height: 8),
                                Text(
                                  'Ledger stock: ${_reconciliation!['stock_quantity'] ?? 0} • Tracked: ${_reconciliation!['tracked_quantity'] ?? 0} • ${_reconciliation!['reconciled'] == true ? 'Reconciled' : 'Registration required'}',
                                ),
                                if (_reconciliation!['reconciled'] != true) ...[
                                  ...
                                  if (_mode == 'serial') ...[
                                    ...
                                    TextField(
                                      ...
                                      decoration: const InputDecoration(
                                        ...
                                        hintText:
                                            'Generated automatically or enter one serial per line',
                                        helperText:
                                            'Manual editing and scanner input remain supported.',
                                        border: OutlineInputBorder(),
                                      ),
                                    ),
                                  ],
                                  if (_mode == 'batch') ...[
                                    ..._batches.asMap().entries.map(
                                      (e) => ListTile(
                                        contentPadding: EdgeInsets.zero,
                                        title: Text(
                                          '${e.value['batch_number']} • Qty ${e.value['quantity']}',
                                        ),
                                        subtitle: Text(
                                          'MFG ${e.value['manufactured_on'] ?? '-'} • EXP ${e.value['expiry_on'] ?? '-'}',
                                        ),
                                        trailing: IconButton(
                                          icon: const Icon(
                                  ...
                                  TextField(
                                    controller: _note,
                                    decoration: const InputDecoration(
                                      labelText: 'Registration note (optional)',
                                      border: OutlineInputBorder(),
                                    ),
                                  ),
                                  const SizedBox(height: 12),
