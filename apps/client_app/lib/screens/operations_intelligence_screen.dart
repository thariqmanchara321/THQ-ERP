...
class _OperationsIntelligenceScreenState
  ...
  final OperationsIntelligenceService _service =
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      final values = await Future.wait<dynamic>([
        ...
        _service.customerCredit(
          tenantId: _tenantId,
          locationId: _locationId,
          query: query,
        ),
        _service.supplierPayables(
          tenantId: _tenantId,
          locationId: _locationId,
          query: query,
        ),
      ...
      setState(() {
        ...
        _selectedReorder.removeWhere(
          (key) => !_reorder.any((row) => _reorderKey(row) == key),
        );
      });
    } catch (error) {
  ...
  String _qty(dynamic value) =>
      _n(value).toStringAsFixed(_n(value) % 1 == 0 ? 0 : 2);
  String _reorderKey(Map<String, dynamic> row) =>
      '${row['location_id']}|${row['variant_id']}';
          ...
          const Expanded(
            child: Column(
              ...
              children: [
                ...
                SizedBox(height: 2),
                Text(
                  'Live cross-module ERP intelligence: stock, credit, purchasing, transfers, offline POS, tracking and restaurant operations.',
                ),
              ],
            ),
          ),
  ...
  Widget _overview() {
    final cards = <(String, String, IconData)>[
      (
        'Low stock',
        '${_attention['low_stock'] ?? 0}',
        Icons.warning_amber_outlined,
      ),
      (
        'Out of stock',
        '${_attention['out_of_stock'] ?? 0}',
        Icons.remove_shopping_cart_outlined,
      ),
      (
        'Dead stock',
        '${_attention['dead_stock'] ?? 0}',
        Icons.inventory_2_outlined,
      ),
      (
        'Inventory value',
        _money(_attention['inventory_value']),
        Icons.warehouse_outlined,
      ),
      (
        'Receivables',
        _money(_attention['receivables']),
        Icons.account_balance_wallet_outlined,
      ),
      (
        'Overdue receivables',
        _money(_attention['overdue_receivables']),
        Icons.schedule_outlined,
      ),
      (
        'Supplier payables',
        _money(_attention['payables']),
        Icons.payments_outlined,
      ),
      (
        'PR approvals',
        '${_attention['purchase_requests_awaiting_approval'] ?? 0}',
        Icons.approval_outlined,
      ),
      (
        'PO approvals',
        '${_attention['purchase_orders_awaiting_approval'] ?? 0}',
        Icons.fact_check_outlined,
      ),
      (
        'Draft GRNs',
        '${_attention['draft_grns'] ?? 0}',
        Icons.inventory_outlined,
      ),
      (
        'Draft purchase invoices',
        '${_attention['draft_purchase_invoices'] ?? 0}',
        Icons.receipt_long_outlined,
      ),
      (
        'Transfers in transit',
        '${_attention['transfers_in_transit'] ?? 0}',
        Icons.local_shipping_outlined,
      ),
      (
        'Offline POS conflicts',
        '${_attention['offline_pos_conflicts'] ?? 0}',
        Icons.sync_problem_outlined,
      ),
      (
        'Batches expiring ≤30d',
        '${_attention['batches_expiring_30d'] ?? 0}',
        Icons.event_busy_outlined,
      ),
      (
        'Warranties expiring ≤30d',
        '${_attention['warranties_expiring_30d'] ?? 0}',
        Icons.verified_user_outlined,
      ),
      (
        'Restaurant open orders',
        '${_attention['restaurant_open_orders'] ?? 0}',
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 8),
                Text(
                  'Demand window: $_days days. These cards now include Purchasing V2 approvals/drafts, warehouse transfers, offline POS conflicts, expiring trace stock/warranties and live restaurant orders. Use the tabs for detailed stock, credit, payables and purchasing analysis.',
                ),
              ],
            ),
          ),
    ...
    rows: _inventory
        .map(
          (r) => [
            ...
            r['sku'],
            _qty(r['available']),
            _qty(r['reorder_level']),
            _qty(r['max_stock']),
            _qty(r['avg_daily_sales']),
            r['days_cover'] == null
                ? '—'
                : '${_n(r['days_cover']).toStringAsFixed(1)} d',
            _qty(r['suggested_reorder']),
    ...
    rows: _customers
        .map(
          (r) => [
            r['customer_name'],
            r['phone'],
            _money(r['total_outstanding']),
            _money(r['credit_limit']),
            _money(r['available_credit']),
            r['utilization_pct'] == null
                ? '—'
                : '${_n(r['utilization_pct']).toStringAsFixed(1)}%',
            _money(r['current_amount']),
            _money(r['days_1_30']),
            _money(r['days_31_60']),
            _money(r['days_61_90']),
            _money(r['days_90_plus']),
            _status(r['status']),
          ],
        )
        .toList(),
        ...
        Padding(
          ...
          child: Row(
            children: [
              Expanded(
                child: Text(
                  '${_reorder.length} reorder suggestions • ${_selectedReorder.length} selected',
                ),
              ),
              FilledButton.icon(
                onPressed: _selectedReorder.isEmpty
                    ? null
                    : _createSelectedPurchaseOrders,
        ...
        Expanded(
          child: _table(
            ...
            rowBuilder: (r) => [
              ...
              r['product_name'],
              r['sku'],
              _qty(r['current_stock']),
              _qty(r['reorder_level']),
              _qty(r['avg_daily_sales']),
              r['days_cover'] == null
                  ? '—'
                  : '${_n(r['days_cover']).toStringAsFixed(1)} d',
              _qty(r['suggested_quantity']),
              r['last_supplier_name'] ?? 'No supplier history',
              _money(r['last_unit_cost']),
            ],
          ),
        ),
      ],
            ...
            trailing: Row(
              ...
              children: [
                ...
                PopupMenuButton<String>(
                  ...
                  itemBuilder: (_) =>
                      _nextStatuses(row['status']?.toString() ?? '')
                          .map(
                            (s) => PopupMenuItem(
                              value: s,
      ...
      if (action == 'approve' || action == 'reject') {
        ...
        final note = await showDialog<String>(
          ...
          builder: (dialogContext) => AlertDialog(
            title: Text(
              approving ? 'Approve purchase order' : 'Reject purchase order',
            ),
            content: TextField(
  ...
  Future<void> _showOrder(String id) async {
    try {
      final detail = await _service.purchaseOrderDetail(
        tenantId: _tenantId,
        purchaseOrderId: id,
      );
      if (!mounted) return;
      ...
      await showDialog<void>(
        ...
        builder: (dialogContext) => AlertDialog(
          title: Text(
            '${order['order_number'] ?? 'Purchase Order'} • ${order['status'] ?? ''}',
          ),
          content: SizedBox(
            width: 760,
            child: SingleChildScrollView(
              child: Column(
                ...
                children: [
                  Text(
                    '${order['supplier_name'] ?? ''} • ${order['location_name'] ?? ''}',
                  ),
                  const SizedBox(height: 12),
                  ...items.map(
                    (i) => ListTile(
                      ...
                      subtitle: Text(
                        '${_qty(i['quantity'])} × ${_money(i['unit_cost'])}',
                      ),
                  ...
                  ...history.map(
                    (h) => ListTile(
                      ...
                      subtitle: Text(
                        '${h['changed_at'] ?? ''}${(h['reason'] ?? '').toString().isEmpty ? '' : ' • ${h['reason']}'}',
                      ),
                    ),
                  ),
                ],
    ...
    try {
      for (final entry in groups.entries) {
        ...
        await _service.createPurchaseOrder(
          ...
          notes:
              'Created from THQ reorder suggestions ($_days-day demand window).',
          items: entry.value
              .map(
                (r) => <String, dynamic>{
          ...
          child: DataTable(
            ...
            rows: List<DataRow>.generate(dataRows.length, (index) {
              ...
              final selected = source == null
                  ? false
                  : _selectedReorder.contains(_reorderKey(source));
              return DataRow(
                selected: selected,
                cells: [
                  if (leading)
  ...
  Widget _status(dynamic value) {
    ...
    final warn =
        text.contains('low stock') || text == 'overdue' || text == 'submitted';
    final color = severe
