...
class ProductIdentifiersScreen extends StatefulWidget {
  ...
  @override
  State<ProductIdentifiersScreen> createState() =>
      _ProductIdentifiersScreenState();
}
...
class _ProductIdentifiersScreenState extends State<ProductIdentifiersScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      if (templateId == null ||
          !templates.any((e) => e['id']?.toString() == templateId)) {
        ...
        templateId =
            (defaults.isNotEmpty
                    ? defaults.first
                    : (templates.isNotEmpty ? templates.first : null))?['id']
                ?.toString();
      }
      setState(() {
        _identifiers = ids;
          ...
          content: SizedBox(
            ...
            child: Column(
              ...
              children: [
                ...
                TextField(
                  controller: label,
                  decoration: const InputDecoration(labelText: 'Label / Note'),
                ),
  ...
  Future<void> _print(ProductIdentifier identifier) async {
    final template = _templates
        .where((e) => e['id']?.toString() == _templateId)
                    ...
                    child: Padding(
                      ...
                      child: Wrap(
                        ...
                        children: [
                          ...
                          if (_canManage)
                            OutlinedButton.icon(
                              onPressed: () => _generate('barcode'),
                              icon: const Icon(Icons.view_week_outlined),
                              label: const Text('Generate Barcode'),
                            ),
                          if (_canManage)
                            OutlinedButton.icon(
                              onPressed: () => _generate('qr'),
                  ...
                  Expanded(
                    child: Card(
                      child: _identifiers.isEmpty
                          ? const Center(
                              child: Text(
                                'No identifiers yet. Add a barcode, QR, manufacturer code, supplier code, internal code or alternate SKU.',
                              ),
                            )
                          : ListView.separated(
                              ...
                              itemBuilder: (context, index) {
                                ...
                                return ListTile(
                                  ...
                                  subtitle: Text(
                                    '${_typeLabel(id.type)}${id.supplierName?.isNotEmpty == true ? ' • ${id.supplierName}' : ''}${id.isPrimary ? ' • PRIMARY' : ''}${!id.active ? ' • ARCHIVED' : ''}${id.label?.isNotEmpty == true ? '\n${id.label}' : ''}',
                                  ),
