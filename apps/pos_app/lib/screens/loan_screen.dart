            ...
            child: SingleChildScrollView(
              child: Column(
                ...
                children: [
                  ...
                  if (counterpartyType == 'customer') DropdownButtonFormField<String>(
                    initialValue: customerId,
                    isExpanded: true,
                    decoration: const InputDecoration(labelText: 'Customer / Client *', border: OutlineInputBorder()),
                    items: _customers.map((c) => DropdownMenuItem(value: c.id, child: Text('${c.name} • ${c.publicId}'))).toList(),
                    onChanged: (value) => setLocal(() => customerId = value),
                  ) else if (counterpartyType == 'supplier') DropdownButtonFormField<String>(
                    initialValue: supplierId,
                    isExpanded: true,
                    decoration: const InputDecoration(labelText: 'Supplier / Lender *', border: OutlineInputBorder()),
                    items: _suppliers.map((x) => DropdownMenuItem(value: x.id, child: Text('${x.name} • ${x.publicId}'))).toList(),
