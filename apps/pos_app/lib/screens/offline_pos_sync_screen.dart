...
class _OfflinePosSyncScreenState extends State<OfflinePosSyncScreen> {
  ...
  OfflineQueueSummary _summary = const OfflineQueueSummary(pending: 0, conflict: 0, error: 0, synced: 0);
  ...
  Future<void> _refresh() async {
    ...
    final summary = await _local.summary(tenantId: _tenantId, deviceId: _deviceId);
    if (!mounted) return;
    setState(() {
      _rows = rows;
      _summary = summary;
  ...
  Future<void> _syncNow() async {
    ...
    try {
      final result = await _sync.syncPending(widget.session, includeConflicts: false);
      try { await _sync.refreshCatalogue(widget.session); } catch (_) {}
      if (!mounted) return;
      setState(() => _message = 'Attempted ${result.attempted} • Synced ${result.synced} • Conflicts ${result.conflicts} • Pending ${result.pending}');
    } catch (error) {
  ...
  Future<void> _retry(OfflineInvoiceRecord row) async {
    ...
    try {
      ...
      await _sync.syncPending(widget.session, includeConflicts: true, onlyRequestId: row.requestId);
    } catch (error) {
      if (mounted) setState(() => _message = error.toString());
    } finally {
      if (mounted) setState(() => _busy = false);
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          ...
          FilledButton.icon(onPressed: _busy ? null : _syncNow, icon: const Icon(Icons.cloud_sync_outlined), label: Text(_busy ? 'Syncing…' : 'Sync Now')),
      ...
      body: Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Wrap(spacing: 8, runSpacing: 8, children: [
            ...
            _Stat('Conflict', _summary.conflict, Icons.warning_amber_outlined),
            _Stat('Error', _summary.error, Icons.error_outline),
            _Stat('Synced', _summary.synced, Icons.cloud_done_outlined),
          ]),
          const SizedBox(height: 12),
          SelectableText('Local database: $path', style: Theme.of(context).textTheme.bodySmall),
          if (_message != null) ...[
            ...
            child: _rows.isEmpty
                ...
                : ListView.separated(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      return ListTile(
                        leading: Icon(Icons.receipt_long_outlined, color: _statusColor(context, row.status)),
                        title: Text(row.localInvoiceNumber),
                        subtitle: Text([
                          row.createdAt.toString().split('.').first,
                          'Attempts ${row.attempts}',
                          if (serverNo != null && serverNo.isNotEmpty) 'Server $serverNo',
                          if (row.conflictCode != null) '${row.conflictCode}: ${row.conflictMessage ?? ''}',
                        ].join(' • ')),
                        trailing: Row(mainAxisSize: MainAxisSize.min, children: [
                          Chip(label: Text(row.status.toUpperCase())),
                          if (row.status == 'conflict' || row.status == 'error')
                            IconButton(onPressed: _busy ? null : () => _retry(row), tooltip: 'Retry', icon: const Icon(Icons.replay)),
                          if (!const {'synced', 'cancelled'}.contains(row.status))
                            IconButton(onPressed: _busy ? null : () => _cancel(row), tooltip: 'Cancel local invoice', icon: const Icon(Icons.cancel_outlined)),
                        ]),
                      );
                    },
                  ),
  ...
  Widget build(BuildContext context) => Card(
        child: Padding(
          ...
          child: Row(mainAxisSize: MainAxisSize.min, children: [
            Icon(icon, size: 18), const SizedBox(width: 8), Text('$label: $value', style: const TextStyle(fontWeight: FontWeight.w700)),
          ]),
        ),
      );
}
