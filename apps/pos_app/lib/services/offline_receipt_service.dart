...
class OfflineReceiptService {
  ...
  Future<String?> printQueuedReceipt({
    ...
    if (device == null) {
      return 'No POS device context is available for printing.';
    }
    final raw = await _local.getMeta(
      'printer:${session.business.id}:${device.deviceId}',
    );
    if (raw is! Map) {
      return 'No cached invoice printer is available. Sync once while online to cache printer settings.';
    }
    final profile = Map<String, dynamic>.from(raw);
    final printerName = profile['printer_name']?.toString().trim() ?? '';
    if (printerName.isEmpty) {
      return 'The cached invoice printer has no printer name.';
    }
    ...
    if (payload['payment_method']?.toString() == 'cash' &&
        profile['cash_drawer_enabled'] == true) {
      ...
      pw.Page(
        ...
        build: (_) => pw.Column(
          ...
          children: [
            ...
            pw.Text(
              'POS: ${session.device?.deviceCode ?? ''} • ${session.device?.locationCode ?? ''}',
            ),
                ...
                child: pw.Column(
                  ...
                  children: [
                    ...
                    pw.Row(
                      ...
                      children: [
                        pw.Text(
                          '${_q(qty)} ${item['unit_code'] ?? ''} × ${_money(session, price)}',
                        ),
            ...
            if (outstanding > 0.005)
              _moneyRow(session, 'ACCOUNT DUE', outstanding),
