revoke execute on function public.gst_document_quote_v520(uuid,text,uuid,uuid,date,text,text,jsonb,numeric,numeric) from anon;
revoke execute on function public.gst_purchase_quote_v520(uuid,uuid,uuid,date,text,text,jsonb,numeric,numeric) from anon;
revoke execute on function public.gst_registration_config_v520(uuid,uuid,date) from anon;
...
comment on function public.gst_ui_contract_v520(uuid) is
'THQ ERP v5.2 GST & Compliance UI contract v2. Includes bootstrap and location-to-registration mapping actions. Provider submission remains disabled.';