  ...
  if v_metric in ('sales','cogs','gross_profit','net_profit') then
    ...
    if v_metric in ('sales','cogs') then
      ...
      from (
        with cur as (
          select coalesce(j.source_type,'manual') source_type,
            sum(case
              when v_metric='gross_profit' and a.system_key='sales_revenue' then jl.credit-j.debit
              else 0 end)::numeric dummy
          ...
          select coalesce(j.source_type,'manual') source_type,
            sum(case
              ...
              else 0 end) filter(where j.entry_date between p_from and p_to)::numeric current_amount,
            sum(case
              when v_metric='gross_profit' and a.system_key='sales_revenue' then jl.credit-jl.debit
              when v_metric='gross_profit' and a.account_type='cogs' then -(jl.debit-jl.credit)
              when v_metric='net_profit' and a.account_type='income' then jl.credit-jl.debit
              when v_metric='net_profit' and a.account_type in ('cogs','expense') then -(jl.debit-jl.credit)
              else 0 end) filter(where j.entry_date between v_prev_from and v_prev_to)::numeric previous_amount
          from public.journal_entries j join public.journal_lines jl on jl.journal_entry_id=j.id
          join public.accounting_accounts a on a.id=jl.account_id and a.tenant_id=j.tenant_id
          where j.tenant_id=p_tenant_id and j.status='posted'
            and j.entry_date between v_prev_from and p_to
            and private.erp_document_scope_allowed(p_tenant_id,j.location_id,p_location_id,'view')
        ...
        select source_type,round(coalesce(current_amount,0),4) current_contribution,round(coalesce(previous_amount,0),4) previous_contribution,
          round(coalesce(current_amount,0)-coalesce(previous_amount,0),4) impact
        from source_values
        order by abs(coalesce(current_amount,0)-coalesce(previous_amount,0)) desc
        limit greatest(1,least(coalesce(p_driver_limit,10),50))
      ) q;

      if v_metric='gross_profit' then
        select coalesce(jsonb_agg(to_jsonb(q) order by abs(q.profit_change) desc),'[]'::jsonb) into v_product_drivers
        from (
          with cur as (select * from private.v600_product_profit_rows(p_tenant_id,p_from,p_to,p_location_id,null)),
          prev as (select * from private.v600_product_profit_rows(p_tenant_id,v_prev_from,v_prev_to,p_location_id,null)),
    ...
    from (
      select a.id account_id,a.code,a.name,a.system_key,a.account_type,
        round(case when a.account_type in ('asset','expense','cogs')
          then coalesce(sum(jl.debit-jl.credit) filter(where j.id is not null),0)
          else coalesce(sum(jl.credit-jl.debit) filter(where j.id is not null),0) end,4) amount
      from public.accounting_accounts a
      left join public.journal_lines jl on jl.account_id=a.id
...
values (260,'6.0.0-build1','Explain Balance Scope Hotfix','Corrects v6.0 balance-metric calculations and component details so only posted journal headers within the requested as-of date and location scope contribute. Period Sales/COGS/Profit calculations were already correctly scoped. Also keeps balance-driver change tied to in-period posted movement.')
