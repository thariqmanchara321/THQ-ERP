...
declare
 ...
 seller_state text;pos text:=nullif(trim(coalesce(p_place_of_supply_code,'')),'');seller_ut boolean:=false;interstate boolean;zero_rated boolean:=false;without_payment boolean:=false;
...
begin
 ...
 if supply in('EXPWP','EXPWOP') then pos:='96';zero_rated:=true;without_payment:=supply='EXPWOP';
 elsif supply in('SEZWP','SEZWOP') then pos:=coalesce(pos,party.place_of_supply_code,party.state_code);zero_rated:=true;without_payment:=supply='SEZWOP';
 else
 ...
 for x in select value from jsonb_array_elements(p_items) loop
  ...
  if prof.id is null then rate:=coalesce(prod.tax_rate,0);cess_rate:=0;cess_unit:=0;inclusive:=false;rcm:=false;taxability:='taxable';hsn:=prod.legacy_hsn;profile_source:='legacy_product';profile_status:='review_required';warnings:=array_append(warnings,'Product '||prod.sku||' has no GST profile; generic legacy tax rate used');
  else rate:=prof.gst_rate;cess_rate:=prof.cess_rate;cess_unit:=prof.cess_per_unit;inclusive:=prof.tax_inclusive;rcm:=prof.reverse_charge;taxability:=prof.taxability;hsn:=prof.hsn_sac;profile_source:=prof.source;profile_status:=prof.validation_status;if prof.validation_status='review_required' then warnings:=array_append(warnings,'Product '||prod.sku||' GST profile requires review');end if;end if;
  ...
  if taxability<>'taxable' or without_payment then rate:=0;cess_rate:=0;cess_unit:=0;end if;
  ...
  if rate>0 then if interstate is true then igst:=round(taxable*rate/100,2);elsif interstate is false then cgst:=round(taxable*(rate/2)/100,2);if seller_ut then utgst:=round(taxable*(rate/2)/100,2);else sgst:=round(taxable*(rate/2)/100,2);end if;end if;end if;
  gst_tax:=cgst+sgst+utgst+igst;line_total:=case when inclusive then round(gross+qty*cess_unit,2) else round(taxable+gst_tax+cess,2) end;
...
insert into public.thq_schema_releases(migration_no,schema_version,release_name,notes) values(216,'5.2.0-foundation','GST Foundation Hotfix 2','Separates zero-rated supply classification from with-payment/without-payment tax treatment for SEZ and exports.') on conflict(migration_no) do update set schema_version=excluded.schema_version,release_name=excluded.release_name,notes=excluded.notes;
commit;