...
class _MobilePosPaymentScreenState extends State<MobilePosPaymentScreen> {
  ...
  void initState() {
    super.initState();
    // v5.2.2 Build 30 always uses authoritative automatic whole-unit round off.
    // Keep the payment page aligned with the server so allocations cover the
    // final invoice total exactly.
    _roundOff.text = _automaticRoundOff.toStringAsFixed(2);
  ...
  void _recalculate() {
    if (!mounted) return;
    final target = _automaticRoundOff.toStringAsFixed(2);
    if (_roundOff.text != target) {
      _roundOff.text = target;
    }
    setState(() {});
  }
