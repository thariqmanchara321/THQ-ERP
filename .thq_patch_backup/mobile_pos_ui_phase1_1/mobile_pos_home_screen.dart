...
class _State extends State<MobilePosHomeScreen> {
  ...
  Future<void> checkout() async {
    ...
    try {
      ...
      // The local invoice stays queued. Automatic sync will retry it.
    }
  ...
  Widget build(BuildContext context) {
    ...
    final showSidebar = size.width >= 980;
    final filteredRows = filtered.where(_matchesProductSection).toList();
  ...
  bool _matchesProductSection(MobileProduct product) {
                  ...
                  child: Column(
                    children: [
                      ...
                      _navItem(
                        ...
                        label: 'Purchase',
                        onTap: () => _moduleQueued('Purchase', inDrawer),
                      ),
                      _navItem(
                        icon: Icons.account_balance_wallet_outlined,
                        label: 'Expense',
                        onTap: () => _moduleQueued('Expense', inDrawer),
                      ),
                      const Padding(
                      ...
                      _navItem(
                        ...
                        label: 'Reports',
                        onTap: () => _moduleQueued('Reports', inDrawer),
                      ),
                      _navItem(
    ...
    if (compact) {
      return SizedBox(
        ...
        child: ListView.separated(
          ...
          itemBuilder: (_, index) => SizedBox(width: 145, child: cards[index]),
        ),
      );
    }
  ...
  Widget _catalogueToolbar({required bool compact}) {
    const sections = ['All', 'Stock', 'Serial', 'Batch', 'Service'];
    return Padding(
          ...
          SizedBox(
            ...
            child: ListView.separated(
              ...
              itemBuilder: (_, index) {
                ...
                final active = _productSection == section;
                return ChoiceChip(
                  selected: active,
                  label: Text(section),
                  onSelected: (_) => setState(() => _productSection = section),
                  showCheckmark: false,
                  selectedColor: const Color(0xFF147AF3),
                  labelStyle: TextStyle(
                    color: active ? Colors.white : const Color(0xFF566477),
                    fontWeight: FontWeight.w700,
                    fontSize: 10.5,
                  ),
                  side: BorderSide(
  ...
  Widget _productGrid(List<MobileProduct> rows, {required bool compact}) =>
      GridView.builder(
        ...
        gridDelegate: SliverGridDelegateWithMaxCrossAxisExtent(
          maxCrossAxisExtent: compact ? 175 : 185,
          mainAxisExtent: compact ? 142 : 148,
          crossAxisSpacing: 9,
          mainAxisSpacing: 9,
        ),
            ...
            Expanded(
              child: cart.isEmpty
                  ...
                  : ListView.separated(
                      ...
                      itemBuilder: (_, index) => _cartLine(cart[index]),
                    ),
            ),
          ...
          Row(
            children: [
              ...
              _quantityButton(
                ...
                tooltip: 'Remove item',
                onPressed: () => setState(() => cart.remove(line)),
              ),
            ],
          ),
        ],
  ...
  Future<void> _payFromCartSheet() async {
  ...
  Widget _mobileCartBar() => Padding(
        padding: const EdgeInsets.fromLTRB(9, 8, 9, 9),
        child: Row(
          children: [
            Expanded(
              child: OutlinedButton.icon(
                onPressed: cart.isEmpty ? null : _openCartSheet,
                icon: const Icon(Icons.shopping_cart_outlined),
                label: Text(
                  cart.isEmpty ? 'Cart empty' : '${cart.length} items • ${money(total)}',
                  maxLines: 1,
                  overflow: TextOverflow.ellipsis,
                ),
                style: OutlinedButton.styleFrom(
