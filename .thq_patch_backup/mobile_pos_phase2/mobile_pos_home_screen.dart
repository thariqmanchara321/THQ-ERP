...
class _State extends State<MobilePosHomeScreen> {
  ...
  Future<void> _syncNow() async {
    ...
    try {
      final result = await sync.sync(widget.session);
      ...
      setState(() {
        if (_manualOffline) {
          syncText = result.conflicts > 0
              ? 'Offline | ${result.conflicts} conflict(s)'
              : 'Offline | manual mode';
        } else if (result.conflicts > 0) {
          syncText = '${result.conflicts} conflict(s) need attention';
        } else if (result.pending > 0 && !catalogueRefreshed) {
          syncText = 'Offline | invoices queued';
        } else {
          syncText = 'Synced';
        }
      });
      if (mounted) {
        ThqNotify.showSnackBar(
          ...
          SnackBar(
            content: Text(
              'Sync: ${result.synced} synced, ${result.pending} pending, '
