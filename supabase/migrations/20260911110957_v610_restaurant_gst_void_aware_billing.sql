...
begin
  ...
  if abs(coalesce(p_round_off,0)) > 0.999999 then
  ...
  v_req_state := private.gst_request_begin_v520(
    p_tenant_id,
    v_wrapper_request,
    'gst.restaurant.order.bill.v520',
    v_req_payload
  );
  ...
  -- v6.1 cancellation-aware authoritative billing:
  -- only remaining active quantity reaches the v5.2 GST Sale writer.
  -- Existing line discount is prorated by the remaining active quantity.
  select coalesce(
    jsonb_agg(
      jsonb_strip_nulls(
        jsonb_build_object(
  ...
  if v_method <> 'credit' and v_total > 0.005 then
    v_payment := public.sales_add_payment_v47(
      ...
      'Restaurant settlement ' || o.order_number,
      v_payment_request
    );

    v_payment_id := nullif(v_payment->>'payment_id','')::uuid;
    if v_payment_id is null then
      raise exception 'Restaurant settlement payment was not created';
    end if;

    select j.id
    into v_payment_journal
    from public.journal_entries j
    where j.tenant_id = p_tenant_id
      and j.source_type = 'sale_payment'
      and j.source_id = v_payment_id
      and j.status = 'posted'
    order by j.created_at desc
    limit 1;

    if v_payment_journal is null then
      raise exception 'Restaurant settlement payment journal was not created';
    end if;
  else
  ...
  v_response := coalesce(v_sale,'{}'::jsonb) || jsonb_build_object(
    ...
    'payment_status', coalesce(
      nullif(v_payment->>'payment_status',''),
      case when v_method='credit' then 'unpaid' else 'paid' end
    ),
  ...
  perform private.business_audit_write_v471(
    p_tenant_id,
    'restaurant.order.bill.gst_v520',
    'restaurant_order',
    o.id,
    o.order_number,
    to_jsonb(o),
