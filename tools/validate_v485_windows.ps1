...
function Invoke-FlutterStep {
    param([Parameter(Mandatory=$true)][string]$Directory,[Parameter(Mandatory=$true)][string[]]$Arguments,[Parameter(Mandatory=$true)][string]$Label)
    Write-Host "`n=== $Label ===" -ForegroundColor Cyan
    Push-Location (Join-Path $Root $Directory)
    try { & flutter @Arguments; if ($LASTEXITCODE -ne 0) { throw "$Label failed with exit code $LASTEXITCODE" } }
    finally { Pop-Location }
}
Write-Host 'THQ ERP v4.8.5 Warehouse & Transfers release validation' -ForegroundColor Green
$python=Get-Command python -ErrorAction SilentlyContinue
if (-not $python) {$python=Get-Command py -ErrorAction SilentlyContinue}
if (-not $python) {throw 'Python is required for tools/verify_v485_release.py.'}
Write-Host "`n=== v4.8.5 static release verification ===" -ForegroundColor Cyan
if ($python.Name -eq 'py.exe' -or $python.Name -eq 'py') { & $python.Source -3 (Join-Path $Root 'tools\verify_v485_release.py') }
else { & $python.Source (Join-Path $Root 'tools\verify_v485_release.py') }
if ($LASTEXITCODE -ne 0) {throw 'v4.8.5 static verification failed.'}
if (-not (Get-Command flutter -ErrorAction SilentlyContinue)) {throw 'Flutter SDK was not found in PATH.'}
$projects=@(
    @{Path='packages\erp_core';Name='erp_core'},@{Path='apps\admin_panel';Name='Admin'},@{Path='apps\client_app';Name='Client'},@{Path='apps\pos_app';Name='POS'}
)
...
Write-Host "`nTHQ ERP v4.8.5 validation completed successfully." -ForegroundColor Green
