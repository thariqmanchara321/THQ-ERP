...
Write-Host 'THQ ERP v4.8.0 local Flutter validation' -ForegroundColor Green
...
foreach ($project in $projects) {
    ...
    Invoke-FlutterStep -Directory $project.Path -Arguments @('analyze') -Label "$($project.Name) analyze"
...
Write-Host "`nTHQ ERP v4.8.0 Flutter validation completed successfully." -ForegroundColor Green
if (-not $Build) {
