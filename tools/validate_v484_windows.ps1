...
Write-Host 'THQ ERP v4.8.4 Purchasing V2 release validation' -ForegroundColor Green
Write-Host "Root: $Root"

$python = Get-Command python -ErrorAction SilentlyContinue
if (-not $python) { $python = Get-Command py -ErrorAction SilentlyContinue }
if (-not $python) { throw 'Python is required for tools/verify_v484_release.py.' }

Write-Host "`n=== v4.8.4 static release verification ===" -ForegroundColor Cyan
if ($python.Name -eq 'py.exe' -or $python.Name -eq 'py') { & $python.Source -3 (Join-Path $Root 'tools\verify_v484_release.py') }
else { & $python.Source (Join-Path $Root 'tools\verify_v484_release.py') }
if ($LASTEXITCODE -ne 0) { throw 'v4.8.4 static verification failed.' }
...
Write-Host "`nTHQ ERP v4.8.4 validation completed successfully." -ForegroundColor Green
